...
1. `main.update(dt)` asks the menu bar first, then the dock if the menu bar
   didn't already consume the click - both float over everything else, and
   a menu hanging off the bar covers the dock in turn. Only then does the
   window manager (`wm.update(dt, chromeConsumed)`) and finally the desktop
   get a look, each told whether an earlier stage already claimed the click
   so a click-through doesn't also land on whatever was underneath.
2. `WindowManager.update()` figures out which window (if any) the click hit,
   focuses it (`bringToFront`), and handles chrome - the three titlebar
   controls (close, minimise, zoom), the resize grip, dragging the titlebar -
   itself. Anything not chrome gets passed down to the focused app as
   `pointer`.
3. Every open window's app gets an `update(dt, pointer)` call each frame, but
   only the focused *and* hovered one gets a live `pointer` - `{x, y, down,
   pressed, released, wheel, hover}`, already translated into the app's own
   0,0 content space. Every other window gets a dead one (`hover: false`,
   nothing down), so a background window's buttons never fire just because
   the mouse happens to be over them. A minimised window keeps running -
   `update` still reaches it - but is never on screen to hit-test against.
4. `draw()` mirrors this: desktop wallpaper and icons, then every
   non-minimised window (oldest to newest, so focus order is stacking
   order), then the dock, then the menu bar on top of all of it, then the
   cursor drawn last so nothing ever covers the pointer.
...
- `icon` - optional, but expected by the dock and menu bar: a name matching
  `assets/icons/<name>.png` (own PNG preferred, then the one Sol ships, see
  "Making it yours" in the README). `Dock.windowsFor()`/`minimizedFor()`
  key off `win.app.icon` to decide whether clicking a dock slot raises an
  existing window or opens a new one, so an app meant to live in the dock
  needs one. `iconImage()` in `assets.ghost` does **not** tolerate a name
  with no shipped PNG at all - `shipped()` calls `new Image(...)` straight
  through with no existence check first, unlike the disk-override tier,
  which does check before loading. Ship a real PNG for any new dock/menu
  app rather than assuming a missing one degrades gracefully; it doesn't.

Nothing here requires extending anything. `apps/app.ghost`'s `App` exists
because several apps kept re-declaring the same handful of fields and a
...
class MyApp extends App {
  constructor(context) {
    super.constructor(context, 'My App', 200, 140, 'myapp')

    // this.context, this.title, this.width, this.height, this.icon are set.
...
draws it as the same bottom strip File Manager's own message bar always
drew, in every app that uses it (Text Edit's "Saved", File Manager's
"Already exists", Pixel Art's "Nothing to save this as yet"). Call
...
1. New file in `apps/`, a class extending `App`, `super.constructor(context,
   title, width, height, icon)` first thing in the constructor.
2. Ship an icon: a 20x20 PNG at `assets/icons/<name>.png`. It can be a
   pixel-exact rasterization of a drawn glyph (see `desktop.ghost`'s
   `drawPixelArtGlyph`-style functions and how `assets/icons/pixelart.png`
   was generated from one) or hand-drawn - just don't skip it, per the
   `iconImage()` note above.
3. Wire it into the three launch points - `desktop.ghost`'s
   `Desktop.buildIcons()` (needs a glyph function too, as the drawn
   fallback for a disk-side icon override gone missing; copy the shape of
   `drawDocumentGlyph`), `dock.ghost`'s `Dock`'s `this.apps` list, and
   `menubar.ghost`'s `Menubar`'s `systemMenu.setItems()` list. All three
   just call `context.wm.open(new YourApp(context))` (the dock's launcher
   wraps that in a function so it can raise an existing window instead).
4. If it opens files from File Manager, extend `FileManager.activate()`'s
   `if (entry.kind == 'folder') ... else if (...) ... else` chain rather
   than replacing it - see how Pixel Art's `.pixel` branch sits ahead of the
   `TextEdit` fallback.
5. Persist through `context.vfs`, never `lumen:filesystem` directly - it
...
- **`theme.ghost`** - every color, font, and metric, plus the shared
  rounded-panel drawing (`drawPanel`, `drawInset`, `drawChip`, `hitTest`).
  Nothing picks its own color; if a new one is needed, it goes in `theme`
  first.
- **`pixelfont.ghost`** - the bitmap font itself, compiled from `.`/`#` rows
  into rectangles at startup. Draw text through `fonts.body`/`fonts.heading`
  - `.print(text, x, y)`, `.printCentered(text, cx, y)`, `.getWidth(text)`,
    `.getHeight()`, `.getLineHeight()` - never `canvas.setFont`/`print`,
  which don't exist here; see the README's "Why the text is drawn as
  rectangles".
- **`assets.ghost`** - PNG loading for the cursor, icons, and wallpapers,
  disk override preferred over the shipped copy. See the `icon` note above
  for the one place this module's null-safety doesn't reach.
- **`widgets.ghost`** - `Button`, `ToggleButton`, `ScrollList`, `TextField`.
  ...
  fit the mold (no label, a selection outline instead of a fill change) and
  got custom hit-testing instead, the same way desktop icons do.
- **`window.ghost`** - `Window` (chrome around one app: titlebar, the three
  controls, resize, minimise, zoom) and `WindowManager` (the list of them,
  focus, drag). This is the one place that reads an app's
  `title`/`width`/`height`/`resizable`/`minWidth`/`minHeight`.
- **`dock.ghost`** / **`menubar.ghost`** - opening and finding apps (the
  dock) and the system/window menus plus the clock (the menu bar). Both
  hold their own hardcoded list of built-in apps rather than discovering
  them, so a new app needs adding to both by hand (see "Adding an app").
...
- **`iconImage()`'s null-safety only covers the disk-override tier.** A name
  with no PNG anywhere on disk *and* no shipped PNG under `assets/icons/`
  crashes the whole desktop the first time something tries to draw it
  (`shipped()` calls `new Image(...)` with nothing checking first) - Ghost
  has no try/catch to recover from that mid-script either, so the only fix
  is to have shipped one in the first place. See "Adding an app" above.
...
round trip without a mouse. For an actual screenshot, hook
`canvas.screenshot('name.png')` into the real `draw()` after a few frames
have passed (`game.screenshotFrame`, incremented each frame and checked for
a fixed value - a bare module-level counter falls into the same "assignment
is scoped to the enclosing function" gotcha the README warns about, so keep
it on `game` like everything else does) rather than calling it from a
one-off manual `draw()`, since it reads back from the renderer after a real
frame has actually been presented. Pull all of this scaffolding back out of
`main.ghost` before committing - it's a debugging aid, not part of the app.
